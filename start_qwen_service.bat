...
powershell -NoProfile -Command "Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -like '*qwen_service.py*' -and $_.CommandLine -like '*--port %PORT%*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }" >nul 2>&1
timeout /t 1 /nobreak >nul
call "%~dp0engines\qwen3tts\.venv\Scripts\python.exe" "%~dp0qwen_service.py" --host 127.0.0.1 --port %PORT% --device cuda:0 --dtype float16
if errorlevel 1 (
  echo.
  echo Qwen Service da thoat voi loi. Nhan phim bat ky de dong cua so nay.
  pause >nul
)
